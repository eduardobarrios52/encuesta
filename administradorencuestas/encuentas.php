				...
				$('.modal-confirm-enc').click(function (e) {
					...
					if ($.trim($('#test').val()) != '') {
						...
						$.ajax({
							...
							type: 'POST',
							data: {test:$.trim($('#test').val()), id:$('#tipencuestas').val()},
							dataType: 'json',
							error: function(err) {
			...
			function agregarPregunta(idenc) {
				...
				$('#tiprespuesta').change(function() {
					...
					$.ajax({
						...
						type: 'POST',
						data: {response:$('#tiprespuesta').val()},
						dataType: 'json',
						error: function(err) {
				...
				$('.modal-confirm-enc').click(function (e) {
					...
					if ($.trim($('#question').val()) != '') {
						...
						$.ajax({
							...
							type: 'POST',
							data: {tipo:$('#tiprespuesta').val(), responses:arrayResponse, question:$('#question').val(), test:idenc, formresp: $('#formrespuesta').val()},
							dataType: 'json',
							error: function(err) {
							...
							success: function(respuesta) {
								...
								if (respuesta != false) {
									...
									if (respuesta == 400) {
										...
										setTimeout(function(){ reloadTable(idenc); }, 2000);
									}
								} else {
				...
				if (tipo == 3) {
					...
					$('#addResponse').click(function() {
						...
						if ($.trim(textresponse) == '' || $.trim(valueresponse) == '') {
							...
							arrayResponse.push({id:('response-'+flagArray),response:textresponse,value:valueresponse});
					...
					$('#addResponse').click(function() {
						...
						if ($.trim(textresponse) == '') {
							...
							arrayResponse.push({id:('response-'+flagArray),response:textresponse});
						...
						$('.modal-confirm-enc').click(function (e) {
							...
							if ($.trim($('#question').val()) != '') {
								...
								$.ajax({
									...
									type: 'POST',
									data: {question:$('#question').val(), id:data},
									dataType: 'json',
									error: function(err) {
									...
									success: function(respuesta) {
										...
										if (respuesta != false) {
											...
											if (respuesta.code == 200) {
												...
												setTimeout(function(){ reloadTable(respuesta.test); }, 2000);
											} else {
					...
					if (operacion == 1) {
						...
						$('.modal-confirm-enc').click(function (e) {
							...
							if (($('#tr-resp-'+data).attr('data-type') == 3 && ($.trim($('#responsedetails').val()) != '' && $.trim($('#valorreponse').val()) != '')) || ($('#tr-resp-'+data).attr('data-type') != 3 && ($.trim($('#responsedetails').val()) != ''))) {
								...
								const params = (($('#tr-resp-'+data).attr('data-type') == 3) ? {response:$('#responsedetails').val(), type:$('#tr-resp-'+data).attr('data-type'), valor:$('#valorreponse').val(), id:data} : {response:$('#responsedetails').val(), type:$('#tr-resp-'+data).attr('data-type'), id:data});
						...
						$('.modal-confirm-enc').click(function (e) {
							...
							if (($('#th-resp-'+data).attr('data-type') == 3 && ($.trim($('#responsedetails').val()) != '' && $.trim($('#valorreponse').val()) != '')) || ($('#th-resp-'+data).attr('data-type') != 3 && ($.trim($('#responsedetails').val()) != ''))) {
								...
								const params = (($('#th-resp-'+data).attr('data-type') == 3) ? {response:$('#responsedetails').val(), type:$('#th-resp-'+data).attr('data-type'), valor:$('#valorreponse').val(), id:data} : {response:$('#responsedetails').val(), type:$('#th-resp-'+data).attr('data-type'), id:data});
							...
							if (respuesta != false) {
								...
								if (respuesta != 400) {
									...
									respuesta.forEach(element => {
										...
										objResponses.push({id:element.H_CVE_PREG,responses:element.RESPONSES,tipo:element.TIPO});
									});
